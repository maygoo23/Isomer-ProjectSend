...
if (isset($_GET["client"])) {
    ...
    if (!empty($this_client)) {
        $page_title .=
            " " .
            __("for client", "cftp_admin") .
            " " .
            html_entity_decode($this_client["name"]);
        $search_on = "client_id";
        $results_type = "client";
    }
}
...
if (isset($_GET["group"])) {
    ...
    if (!empty($group["name"])) {
        $page_title .=
            " " .
            __("for group", "cftp_admin") .
            " " .
            html_entity_decode($group["name"]);
        $search_on = "group_id";
        $results_type = "group";
    }
}
...
if (isset($_GET["category"])) {
    ...
    if (!empty($this_category)) {
        $page_title .=
            " " .
            __("on category", "cftp_admin") .
            " " .
            html_entity_decode($this_category["name"]);
        $results_type = "category";
    }
}

// Setting the filter options to avoid duplicates
$filter_options_uploader = [
    "0" => __("Uploader", "cftp_admin"),
];
$sql_uploaders = $dbh->prepare(
    "SELECT uploader FROM " . TABLE_FILES . " GROUP BY uploader"
);
$sql_uploaders->execute();
$sql_uploaders->setFetchMode(PDO::FETCH_ASSOC);
while ($data_uploaders = $sql_uploaders->fetch()) {
    $filter_options_uploader[$data_uploaders["uploader"]] =
        $data_uploaders["uploader"];
}

$filter_options_assigned = [
    ...
    if (!empty($_POST["batch"])) {
        ...
        switch ($_POST["action"]) {
            case "hide":
                ...
                $flash->success(
                    __(
                        "The selected files were marked as hidden.",
                        "cftp_admin"
                    )
                );
                break;
            case "show":
                ...
                $flash->success(
                    __(
                        "The selected files were marked as visible.",
                        "cftp_admin"
                    )
                );
                break;
            ...
            case "unassign":
                ...
                $flash->success(
                    __(
                        "The selected files were successfully unassigned.",
                        "cftp_admin"
                    )
                );
                break;
            case "delete":
                ...
                if ($delete_results["success"] > 0) {
                    $flash->success(
                        __("The selected files were deleted.", "cftp_admin")
                    );
                }
                if ($delete_results["errors"] > 0) {
                    $flash->error(
                        __("Some files could not be deleted.", "cftp_admin")
                    );
                }
                break;
            case "edit":
...
if ($query_table_files === true) {
    ...
    $cq =
        "SELECT files.*, ( SELECT COUNT(file_id) FROM " .
        TABLE_DOWNLOADS .
        " WHERE " .
        TABLE_DOWNLOADS .
        ".file_id=files.id " .
        $add_user_to_query .
        ") as download_count FROM " .
        TABLE_FILES .
        " files";
    ...
    if (isset($_GET["search"]) && !empty($_GET["search"])) {
        $conditions[] =
            "(filename LIKE :name OR description LIKE :description)";
    ...
    if (isset($_GET["assigned"]) && !empty($_GET["assigned"])) {
        if (array_key_exists($_GET["assigned"], $filter_options_assigned)) {
            ...
            $statement = $dbh->prepare(
                "SELECT DISTINCT file_id FROM " . TABLE_FILES_RELATIONS
            );
            $statement->execute();
    ...
    $pagination_start =
        ($pagination_page - 1) * get_option("pagination_results_per_page");
    ...
    if (isset($no_results_error)) {
        switch ($no_results_error) {
            ...
            case "category":
                $flash->error(
                    __(
                        "There are no files assigned to this category.",
                        "cftp_admin"
                    )
                );
                break;
            ...
            case "account_level":
                $flash->error(
                    __("You have not uploaded any files yet.", "cftp_admin")
                );
                break;
        }
    } else {
    ...
    $header_action_buttons = [
        [
            ...
            "data-attributes" => [
                ...
                "folder-url" =>
                    BASE_URI . "manage-files.php?folder_id={folder_id}",
            ],
        ],
        [
            "url" => "upload.php",
...
if (defined("CURRENT_USER_LEVEL") && CURRENT_USER_LEVEL != "0") {
    ...
    if ($results_type == "global") {
        $filters_form["items"] = [
            "uploader" => [
                "current" => isset($_GET["uploader"])
                    ? $_GET["uploader"]
                    : null,
                "options" => $filter_options_uploader,
            ],
            "assigned" => [
                "current" => isset($_GET["assigned"])
                    ? $_GET["assigned"]
                    : null,
...
if (defined("CURRENT_USER_LEVEL") && CURRENT_USER_LEVEL != "0") {
    ...
    if (!isset($search_on)) {
        $bulk_actions_items["hide_everyone"] = __(
            "Set to hidden from everyone already assigned",
            "cftp_admin"
        );
        $bulk_actions_items["show_everyone"] = __(
            "Set to visible to everyone already assigned",
            "cftp_admin"
        );
    }
}
if (defined("CURRENT_USER_LEVEL")) {
    if (CURRENT_USER_LEVEL != "0" && isset($search_on)) {
        ...
        if (
            ...
            (CURRENT_USER_LEVEL == "0" &&
                get_option("clients_can_delete_own_files") == "1")
        ) {
        ...
        <div class="col-12">
            <?php if ($count_for_pagination > 0) {
                ...
                $conditions = [
                    ...
                    "can_set_expiration" =>
                        CURRENT_USER_LEVEL != "0" ||
                        get_option("clients_can_set_expiration_date") == "1"
                            ? true
                            : false,
                    "can_set_categories" =>
                        CURRENT_USER_LEVEL != "0" ||
                        get_option("clients_can_set_categories") == "1"
                            ? true
                            : false,
                    "total_downloads" =>
                        CURRENT_USER_LEVEL != "0" && !isset($search_on)
                            ? true
                            : false,
                ...
                $thead_columns = [
                    ...
                    [
                        ...
                        "hide" => "phone",
                        "condition" =>
                            $conditions["is_not_client"] &&
                            !$conditions["is_search_on"],
                    ],
                    [
                ...
                while ($row = $sql->fetch()) {
                    ...
                    switch ($results_type) {
                        case "client":
                            $hidden =
                                $assignations["clients"][$this_id]["hidden"];
                            break;
                        case "group":
                            $hidden =
                                $assignations["groups"][$this_id]["hidden"];
                            break;
                    }
                    ...
                    // Is file assigned?
                    $assigned_class =
                        $count_assignations == 0 ? "danger" : "success";
                    $assigned_status =
                        $count_assignations == 0
                            ? __("No", "cftp_admin")
                            : __("Yes", "cftp_admin");
                    ...
                    $categories_list = "";
                    $statement = $dbh->prepare(
                        "SELECT c.name as category_name, c.id as category_id, r.id as rel_id FROM " .
                            TABLE_CATEGORIES_RELATIONS .
                            " r INNER JOIN " .
                            TABLE_CATEGORIES .
                            " c on r.cat_id = c.id WHERE file_id = :file_id"
                    );
                    $statement->bindParam(
                        ":file_id",
                        $file->id,
                    ...
                    $tbody_cells = [
                        ...
                        [
                            "content" => !empty($download_count_content)
                                ? $download_count_content
                                : false,
                            "condition" => $conditions["is_search_on"],
                        ],
                        ...
                        [
                            "content" =>
                                ...
                                $file->id .
                                '" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i><span class="button_label">' .
                                __("Edit", "cftp_admin") .
                                "</span></a>",
                        ],
                    ];
