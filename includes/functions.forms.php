...
function show_search_form($action = "")
{
	...
	<form action="<?php echo $action; ?>" name="form_search" method="get" class="row row-cols-lg-auto g-3 align-items-center">
		<?php echo form_add_existing_parameters(["search", "action"]); ?>
        <div class="col-6 col-md-12">
			<input type="text" name="search" id="search" value="<?php if (
       isset($_GET["search"]) &&
       !empty($_GET["search"])
   ) {
       echo html_output($_GET["search"]);
   } ?>" class="form-control-short form_actions_search_box form-control" />
		</div>
        <div class="col-6 col-md-12">
    		<button type="submit" id="btn_proceed_search" class="btn btn-md btn-pslight"><?php _e(
          "Search",
          "cftp_admin"
      ); ?></button>
        </div>
...
function show_filters_form($arguments)
{
    ...
    $ignore = !empty($arguments["ignore_form_parameters"])
        ? $arguments["ignore_form_parameters"]
        : array_keys($arguments["items"]);
    if (!empty($arguments["hidden_inputs"])) {
        $ignore = array_merge($ignore, array_keys($arguments["hidden_inputs"]));
    }
    ?>
    <form action="<?php echo $arguments[
        "action"
    ]; ?>" name="actions_filters" method="get" class="row row-cols-lg-auto g-3 align-items-end justify-content-end form_filters mt-4 mt-md-0">
        <?php echo form_add_existing_parameters($ignore); ?>
        <?php if (!empty($arguments["hidden_inputs"])) {
            ...
            <div class="col-4 col-md-12">
                <select class="form-select" name="<?php echo $name; ?>" id="<?php echo $name; ?>">
                    ...
                    if (!empty($data["placeholder"])) { ?>
                            <option value="<?php echo $data["placeholder"][
                                "value"
    ...
    echo 'selected="selected"';
        ...
        <?php } ?>
        <div class="col-4 col-md-12">
            <button type="submit" class="btn btn-md btn-pslight"><?php _e(
                "Filter",
                "cftp_admin"
            ); ?></button>
        </div>
...
function show_actions_form($actions)
{
    ...
    <div class="col-6 col-md-12">
        <button type="submit" id="do_action" class="btn btn-md btn-pslight"><?php _e(
            "Proceed",
            "cftp_admin"
        ); ?></button>
    </div>
        ...
        switch ($link) {
            case "homepage":
                echo '<p><a href="' .
                    BASE_URI .
                    '">' .
                    __("Go back to the homepage", "cftp_admin") .
                    "</a></p>";
                break;

            case "password_recovery":
                echo "<p>" .
                    __("Forgot your password?", "cftp_admin") .
                    ' <a href="' .
                    BASE_URI .
                    'reset-password.php">' .
                    __("Set up a new one.", "cftp_admin") .
                    "</a></p>";
                break;

            case "register":
                // Self-registration disabled - admin managed contacts only
                echo "<p>" .
                    tm(
                        __(
            ...
            case "public_page":
                if (get_option("public_listing_home_show_link") == "1") {
                    echo '<p><a href="' .
                        BASE_URI .
                        'public.php">' .
                        __("Browse our public files repository", "cftp_admin") .
                        "</a></p>";
                }
                break;
        }
