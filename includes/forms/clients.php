 ...
 */
$name_placeholder = __(
    "Will be visible on the contact's file list",
    "cftp_admin"
);
...
switch ($clients_form_type) {
    ...
    case "new_client":
        $submit_value = __("Add contact", "cftp_admin");
        $disable_user = false;
        $require_pass = true;
        ...
        $extra_fields = true;
        $group_field = true;
        $group_label = __("Projects", "cftp_admin");
        $ignore_size = false;
        break;
    /** User is editing an existing client */
    case "edit_client":
        $submit_value = __("Save contact", "cftp_admin");
        $disable_user = true;
        $require_pass = false;
    ...
    //         $submit_value =
    //             get_option("clients_auto_approve") == 1
    //                 ? __("Create account", "cftp_admin")
    //                 : __("Request account", "cftp_admin");
    //         $disable_user = false;
    //         $require_pass = true;
    ...
    //             $clients_can_select_group == "all"
    //         ) {
    //             $group_field = true;
    //             $group_label = __("Request access to projects", "cftp_admin");
    //         }
    //         break;
    ...
    //             $group_field = true;
    //             $group_label = __("Request access to projects", "cftp_admin");
    //             $override_groups_list = !empty(
    //                 $found_requests[$client_id]["group_ids"]
    //             )
    //                 ? $found_requests[$client_id]["group_ids"]
    //                 : null;
    //         }
...
<form action="<?php echo html_output(
    $form_action
); ?>" name="client_form" id="client_form" method="post" class="form-horizontal" data-form-type="<?php echo $clients_form_type; ?>">
    <?php addCsrf(); ?>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <input type="text" name="name" id="name" class="form-control required" value="<?php echo isset(
                $client_arguments["name"]
            )
                ? format_form_value($client_arguments["name"])
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <input type="text" name="username" id="username" class="form-control <?php if (
                !$disable_user
            ) {
                echo "required";
            } ?>" maxlength="<?php echo MAX_USER_CHARS; ?>" value="<?php echo isset(
    $client_arguments["username"]
)
    ? format_form_value($client_arguments["username"])
    : ""; ?>" <?php if ($disable_user) {
    echo "readonly";
} ?> placeholder="<?php _e(
    ...
    <div class="form-group row">
        <label for="password" class="col-sm-4 control-label"><?php _e(
            "Password",
            "cftp_admin"
        ); ?></label>
        <div class="col-sm-8">
            <div class="input-group">
                <input type="password" name="password" id="password" class="form-control attach_password_toggler <?php if (
                    $require_pass
                ) {
                    echo "required";
                } ?>" maxlength="<?php echo MAX_PASS_CHARS; ?>" />
            </div>
            <button type="button" name="generate_password" id="generate_password" class="btn btn-light btn-sm btn_generate_password" data-ref="password" data-min="<?php echo MAX_GENERATE_PASS_CHARS; ?>" data-max="<?php echo MAX_GENERATE_PASS_CHARS; ?>"><?php _e(
    "Generate",
    "cftp_admin"
); ?></button>
    ...
    <div class="form-group row">
        <label for="email" class="col-sm-4 control-label"><?php _e(
            "E-mail",
            "cftp_admin"
        ); ?></label>
        <div class="col-sm-8">
            <input type="email" name="email" id="email" class="form-control required" value="<?php echo isset(
                $client_arguments["email"]
            )
                ? format_form_value($client_arguments["email"])
                : ""; ?>" placeholder="<?php _e(
    ...
    in_array($group["id"], $selected_groups)
) {
    echo ' selected="selected"';
} ?>><?php echo $group["name"]; ?></option>
                        <?php } ?>
                    </select>
                    <?php if (!empty($role) && in_array($role, [8, 9])) { ?>
                        <div class="select_control_buttons">
                            <button type="button" class="btn btn-pslight add-all" data-target="groups-select"><?php _e(
                                "Add all",
                                "cftp_admin"
                            ); ?></button>
                            <button type="button" class="btn btn-pslight remove-all" data-target="groups-select"><?php _e(
                                "Remove all",
                                "cftp_admin"
                            ); ?></button>
                        </div>
    ...
    <?php }
        ...
        <div class="form-group row">
            <div class="col-sm-8 offset-sm-4">
                <label for="active">
                    <input type="checkbox" name="active" id="active" <?php echo isset(
                        $client_arguments["active"]
                    ) && $client_arguments["active"] == 1
                        ? 'checked="checked"'
                        : ""; ?>> <?php echo tm(
    __("Active (contact can log in)", "cftp_admin")
); ?>
                </label>
            </div>
        </div>
    ...
    <div class="form-group row">
        <div class="col-sm-8 offset-sm-4">
            <label for="notify_upload">
                <input type="checkbox" name="notify_upload" id="notify_upload" <?php echo isset(
                    $client_arguments["notify_upload"]
                ) && $client_arguments["notify_upload"] == 1
                    ? 'checked="checked"'
                    : ""; ?>> <?php _e(
    "Notify new uploads by e-mail",
    "cftp_admin"
); ?>
            </label>
        </div>
    ...
    <?php if ($clients_form_type == "new_client") { ?>
        <div class="form-group row">
            <div class="col-sm-8 offset-sm-4">
                <label for="notify_account">
                    <input type="checkbox" name="notify_account" id="notify_account" <?php echo isset(
                        $client_arguments["notify_account"]
                    ) && $client_arguments["notify_account"] == 1
                        ? 'checked="checked"'
                        : ""; ?>> <?php _e(
    "Send welcome email",
    "cftp_admin"
        ...
        <div class="form-group row">
            <div class="col-sm-8 offset-sm-4">
                <label for="require_password_change">
                    <input type="checkbox" name="require_password_change" id="require_password_change" <?php echo isset(
                        $client_arguments["require_password_change"]
                    ) && $client_arguments["require_password_change"] == 1
                        ? 'checked="checked"'
                        : ""; ?>> <?php _e(
