...
class Emails
{
    ...
    {
        $this->header =
            get_option("email_header_footer_customize") == "1"
                ? get_option("email_header_text")
                : file_get_contents(
                    EMAIL_TEMPLATES_DIR . DS . EMAIL_TEMPLATE_HEADER
                );
        $this->footer =
            get_option("email_header_footer_customize") == "1"
                ? get_option("email_footer_text")
                : file_get_contents(
                    EMAIL_TEMPLATES_DIR . DS . EMAIL_TEMPLATE_FOOTER
                );
    }
    ...
    {
        $uri_note = __(
            "The login link (to be used as href on a link tag)",
            "cftp_admin"
        );
        ...
        switch ($email_type) {
            case "header_footer":
                ...
                $options = [
                    ...
                    "checkboxes" => ["email_header_footer_customize"],
                    "items" => [],
                ];
                break;
            case "new_files_by_user":
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to a client whenever a new file has been assigned to their account.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_new_file_by_user_subject_customize"
                        ),
                        "subject_text" => get_option(
                            "email_new_file_by_user_subject"
                        ),
                        "body_check" => get_option(
                            "email_new_file_by_user_customize"
                        ),
                        "body_text" => get_option(
            ...
            case "new_files_by_client":
                ...
                $strings = [
                    "subject" =>
                        ...
                        !empty(get_option("email_new_file_by_client_subject"))
                            ? get_option("email_new_file_by_client_subject")
                            : __("New files uploaded by clients", "cftp_admin"),
                    "body" => __(
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to the system administrator whenever a client uploads a new file.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_new_file_by_client_subject_customize"
            ...
            case "new_client":
                ...
                $strings = [
                    "subject" =>
                        ...
                        !empty(get_option("email_new_client_by_user_subject"))
                            ? get_option("email_new_client_by_user_subject")
                            : __("Welcome to ProjectSend", "cftp_admin"),
                    "body" => __(
                        "A new account was created for you. From now on, you can access the files that have been uploaded under your account using the following credentials:",
                        "cftp_admin"
                    ),
                    "body2" => __(
                        "You can log in following this link",
                        "cftp_admin"
                    ),
                    "body3" => __(
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to the new client after an administrator has created their new account. It would be best to include the log in details (username and password).",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_new_client_by_user_subject_customize"
            ...
            case "new_client_self":
                ...
                $strings = [
                    "subject" =>
                        ...
                        !empty(get_option("email_new_client_by_self_subject"))
                            ? get_option("email_new_client_by_self_subject")
                            : __("A new client has registered", "cftp_admin"),
                    "body" => __(
                        "A new account was created using the self registration form on your site. Registration information:",
                        "cftp_admin"
                    ),
                    "label_name" => __("Full name", "cftp_admin"),
                    "label_user" => __("Username", "cftp_admin"),
                    "label_request" => __(
                        "Additionally, the client requests access to the following group(s)",
                        "cftp_admin"
                    ),
                    "body2" => __(
                        "Auto-approvals of new accounts are currently enabled.",
                        "cftp_admin"
                    ),
                    "body3" => __(
                        "You can log in to manually deactivate it.",
                        "cftp_admin"
                    ),
                ];
                if (get_option("clients_auto_approve") == "0") {
                    $strings["body2"] = __(
                        "Please log in to process the request.",
                        "cftp_admin"
                    );
                    $strings["body3"] = __(
                        "Remember, your new client will not be able to log in until an administrator has approved their account.",
                        "cftp_admin"
                    );
                }
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to the system administrator after a new client has created a new account for himself.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_new_client_by_self_subject_customize"
                        ...
                        "tags" => [
                            "%FULLNAME%" => __(
                                "The full name the client registered with",
                                "cftp_admin"
                            ),
                            "%USERNAME%" => __(
                                "The new username for this account",
                            ...
                            "%URI%" => $uri_note,
                            "%GROUPS_REQUESTS%" => __(
                                "List of groups that the client requests membership to",
                                "cftp_admin"
                            ),
                        ],
                    ],
            ...
            case "account_approve":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_account_approve_subject_customize") ==
                            1 &&
                        !empty(get_option("email_account_approve_subject"))
                            ? get_option("email_account_approve_subject")
                            : __("You account has been approved", "cftp_admin"),
                    "body" => __(
                        "Your account has been approved.",
                        "cftp_admin"
                    ),
                    "title_memberships" => __(
                        "Additionally, your group membership requests have been processed.",
                        "cftp_admin"
                    ),
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to a client that requested an account if it gets approved. Group membership requests are also mentioned on the email, separated by their approval status.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_account_approve_subject_customize"
                        ...
                        "tags" => [
                            "%GROUPS_APPROVED%" => __(
                                "List of approved group memberships",
                                "cftp_admin"
                            ),
                            "%GROUPS_DENIED%" => __(
                                "List of denied group memberships",
                                "cftp_admin"
                            ),
                            "%URI%" => $uri_note,
                        ],
            ...
            case "account_deny":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_account_deny_subject_customize") ==
                            1 &&
                        !empty(get_option("email_account_deny_subject"))
                            ? get_option("email_account_deny_subject")
                            : __("You account has been denied", "cftp_admin"),
                    "body" => __(
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to a client that requested an account if it gets denied. All group membership requests for this account are denied automatically.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_account_deny_subject_customize"
            ...
            case "new_user":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_new_user_subject_customize") == 1 &&
                        !empty(get_option("email_new_user_subject"))
                            ? get_option("email_new_user_subject")
                            : __("Welcome to ProjectSend", "cftp_admin"),
                    "body" => __(
                        "A new account was created for you. From now on, you can access the system administrator using the following credentials:",
                        "cftp_admin"
                    ),
                    "body2" => __("Access the system panel here", "cftp_admin"),
                    "body3" => __(
                        "Thank you for using ProjectSend.",
                        "cftp_admin"
                    ),
                    "label_user" => __("Your username", "cftp_admin"),
                    "label_pass" => __("Your password", "cftp_admin"),
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to the new system user after an administrator has created their new account. It would be best to include the log in details (username and password).",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_new_user_subject_customize"
            ...
            case "password_reset":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_pass_reset_subject_customize") == 1 &&
                        !empty(get_option("email_pass_reset_subject"))
                            ? get_option("email_pass_reset_subject")
                            : __("Password reset instructions", "cftp_admin"),
                    "body" => __(
                    ...
                    "body3" => __(
                        "The request is valid only for 24 hours.",
                        "cftp_admin"
                    ),
                    "body4" => __(
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to a user or client when they try to reset their password.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_pass_reset_subject_customize"
                        ...
                        "tags" => [
                            ...
                            "%TOKEN%" => __(
                                "The text string unique to this request. Must be included somewhere.",
                                "cftp_admin"
                            ),
                            "%URI%" => __(
                                "The link to continue the process (to be used as href on a link tag)",
                                "cftp_admin"
                            ),
                        ],
                    ],
            ...
            case "client_edited":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_client_edited_subject_customize") ==
                            1 &&
                        !empty(get_option("email_client_edited_subject"))
                            ? get_option("email_client_edited_subject")
                            : __(
                                "A client has changed memberships requests",
                                "cftp_admin"
                            ),
                    "body" => __(
                        "A client on your site has just changed their groups membership requests and needs your approval.",
                        "cftp_admin"
                    ),
                    "label_name" => __("Full name", "cftp_admin"),
                    "label_user" => __("Username", "cftp_admin"),
                    "label_request" => __(
                        "The client requests access to the following group(s)",
                        "cftp_admin"
                    ),
                    "body2" => __(
                        "Please log in to process the request.",
                ...
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "This email will be sent to the system administrator when a client edits their account and changes the public groups membership requests.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_client_edited_subject_customize"
            ...
            case "2fa_code":
                ...
                $strings = [
                    "subject" =>
                        get_option("email_2fa_code_subject_customize") == 1 &&
                        !empty(get_option("email_2fa_code_subject"))
                            ? get_option("email_2fa_code_subject")
                            : __("Your login verification code", "cftp_admin"),
                    "body" => __(
                        "Someone has attempted to log in to your account.",
                        "cftp_admin"
                    ),
                    "body2" => __(
                        "To continue the process, you need to enter the following code:",
                        "cftp_admin"
                    ),
                    ...
                    "body3" => __(
                        "If you did not initiate the log in process, please change your password as soon as possible.",
                        "cftp_admin"
                    ),
                ];
                $options = [
                    ...
                    "items" => [
                        ...
                        "description" => __(
                            "If the corresponding option is enabled, this email with a one-time security code will be sent to a user when they attempt to log in.",
                            "cftp_admin"
                        ),
                        "subject_check" => get_option(
                            "email_2fa_code_subject_customize"
                        ...
                        "tags" => [
                            "%CODE%" => __(
                                "The 6-digits security code",
                                "cftp_admin"
                            ),
                            "%LOCATION%" => __(
                                "Geographical location from where the user logged in",
                                "cftp_admin"
                            ),
    ...
    private function prepareBody($type)
    {
        ...
        $content =
            $customize_body == "1" && !empty(get_option($body_text_option))
                ...
                : file_get_contents(EMAIL_TEMPLATES_DIR . DS . $filename);
     ...
     */
    private function email_new_client_self(
        $username,
        $fullname,
        $memberships_requests
    ) {
        ...
        if (!empty($memberships_requests)) {
            ...
            $this->email_body = str_replace(
                ["%LABEL_REQUESTS%", "%GROUPS_REQUESTS%"],
                [$strings["label_request"], $this->groups_list],
                $this->email_body
            );
        } else {
            $this->email_body = str_replace(
                ["%LABEL_REQUESTS%", "%GROUPS_REQUESTS%"],
                [__("No group requests made", "cftp_admin"), null],
                $this->email_body
            );
        }
        return [
     ...
     */
    private function email_account_approve(
        $username,
        $name,
        $memberships_requests
    ) {
    ...
    private function email_account_deny($username, $name)
    {
        ...
        $this->email_body = str_replace(
            ["%SUBJECT%", "%BODY1%", "%BODY2%"],
            [$strings["subject"], $strings["body"], $strings["body2"]],
            $this->email_body
        );
        return [
            "subject" => $strings["subject"],
    ...
    private function email_new_files_by_client($files_list)
    {
        ...
        $this->email_body = str_replace(
            ["%SUBJECT%", "%BODY1%", "%FILES%", "%BODY2%", "%BODY3%", "%URI%"],
            [
                $strings["subject"],
                $strings["body"],
                $files_list,
     ...
     *  membership requests.
     */
    private function email_client_edited(
        $username,
        $fullname,
        $memberships_requests
    ) {
    ...
    public function send($arguments)
    {
        ...
        $this->preview = !empty($arguments["preview"])
            ? $arguments["preview"]
            : false;
        $this->type = $arguments["type"];
        $this->addresses = !empty($arguments["address"])
            ? $arguments["address"]
            : "";
        $this->username = !empty($arguments["username"])
            ? $arguments["username"]
            : "";
        $this->password = !empty($arguments["password"])
            ? $arguments["password"]
            : "";
        $this->client_id = !empty($arguments["client_id"])
            ? $arguments["client_id"]
            : "";
        $this->name = !empty($arguments["name"]) ? $arguments["name"] : "";
        $this->files_list = !empty($arguments["files_list"])
            ? $arguments["files_list"]
            : "";
        ...
        switch ($this->type) {
            ...
            case "generic":
                $subject = !empty($arguments["subject"])
                    ? $arguments["subject"]
                    : sprintf(
                        __("Sent from %s", "cftp_admin"),
                        get_option("this_install_title")
                    );
            ...
            case "2fa_code":
                ...
                foreach ($items as $item) {
                    $body_variables[] = !empty($arguments[$item])
                        ? $arguments[$item]
                        : "";
                }
                break;
        ...
        $this->mail_info = call_user_func_array(
            [$this, $this->method],
            $body_variables
        );
        ...
        if ($this->preview == true) {
            ...
            $email->SMTPOptions = [
                "ssl" => [
                    "verify_peer" =>
                        !empty(get_option("mail_ssl_verify_peer")) &&
                        get_option("mail_ssl_verify_peer") == 1
                            ? true
                            : false,
                    "verify_peer_name" =>
                        !empty(get_option("mail_ssl_verify_peer_name")) &&
                        get_option("mail_ssl_verify_peer_name") == 1
                            ? true
                            : false,
                    "allow_self_signed" =>
                        !empty(get_option("mail_ssl_allow_self_signed")) &&
                        get_option("mail_ssl_allow_self_signed") == 1
                            ? true
                            : false,
                ],
            ];
            ...
            switch ($_ENV["MAIL_SYSTEM_USE"] ?? get_option("mail_system_use")) {
                case "smtp":
                    $email->IsSMTP();
                    $email->Host =
                        $_ENV["MAIL_SMTP_HOST"] ?? get_option("mail_smtp_host");
                    $email->Port =
                        $_ENV["MAIL_SMTP_PORT"] ?? get_option("mail_smtp_port");
                    $email->Username =
                        $_ENV["MAIL_SMTP_USER"] ?? get_option("mail_smtp_user");
                    $email->Password =
                        $_ENV["MAIL_SMTP_PASS"] ?? get_option("mail_smtp_pass");
            ...
            $email->AltBody = __(
                "This email contains HTML formatting and cannot be displayed right now. Please use an HTML compatible reader.",
                "cftp_admin"
            );

            $email->SetFrom(
                get_option("admin_email_address"),
                get_option("mail_from_name")
            );
            $email->AddReplyTo(
                get_option("admin_email_address"),
                get_option("mail_from_name")
            ...
            $send = $email->Send();
            $this->debug_result = !empty($_SESSION["email_debug_message"])
                ? $_SESSION["email_debug_message"]
                : null;
