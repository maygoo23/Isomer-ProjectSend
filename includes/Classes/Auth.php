...
class Auth
{
    ...
    public function validate2faRequest($token, $code)
    {
        ...
        if ($user->isActive()) {
            ...
            // Check for stored redirect after 2FA
            $redirect_location = $user->isClient()
                ? CLIENT_VIEW_FILE_LIST_URL
                : BASE_URI . "dashboard.php";

            if (
                isset($_SESSION["redirect_after_2fa"]) &&
                !empty($_SESSION["redirect_after_2fa"])
            ) {
                $stored_url = $_SESSION["redirect_after_2fa"];
                if (
                    strpos($stored_url, "/my_files/") === 0 ||
                    strpos($stored_url, "my_files/") === 0
                ) {
                    $redirect_location = BASE_URI . ltrim($stored_url, "/");
                }
            ...
            if (password_verify($password, $user->getRawPassword())) {
                if ($user->isActive()) {
                    ...
                    if ($new2fa->requires2fa()) {
                        $request2fa = json_decode(
                            $new2fa->requestNewCode($user->id)
                        );
                        if ($request2fa->status == "success") {
                            // Store the redirect URL in session for after 2FA completion
                            if (isset($_SESSION["redirect_after_login"])) {
                                $_SESSION["redirect_after_2fa"] =
                            ...
                            $results = [
                                ...
                                "location" =>
                                    BASE_URI .
                                    "index.php?form=2fa_verify&token=" .
                                    $request2fa->token,
                            ];
                        } else {
                    ...
                    if (
                        ...
                        // Validate that the redirect is safe (within your domain)
                        $stored_url = $_SESSION["redirect_after_login"];
                        if (
                            strpos($stored_url, "/my_files/") === 0 ||
                            strpos($stored_url, "my_files/") === 0
                        ) {
                            $redirect_location =
                                BASE_URI . ltrim($stored_url, "/");
                        }
    ...
    public function socialLogin($provider)
    {
        ...
        if ($count_user > 0) {
            ...
            $username = !username_exists($email_parts[0])
                ? $email_parts[0]
                : generate_username($email_parts[0]);
            ...
            $new_client->set([
                ...
                "account_requested" =>
                    get_option("clients_auto_approve") == 0 ? 1 : 0,
                "type" => "new_client",
                "recaptcha" => null,
            ]);
    ...
    public function loginLdap($email, $password, $language)
    {
        ...
        try {
            ...
            $return = [
                "status" => "error",
                "message" => sprintf(
                    __("LDAP connection error: %s", "cftp_admin"),
                    $e->getMessage()
                ),
            ];
        ...
        ldap_set_option(
            ...
            get_option("ldap_protocol_version")
        );
                    ...
                    if (ldap_bind($ldap, $entries[0]["dn"], $password)) {
                        ...
                        */
                        ...
                        $return = [
                            "status" => "error",
                            "message" => __(
                                "The supplied email or password does not match an existing record.",
                                "cftp_admin"
                            ),
                        ];
