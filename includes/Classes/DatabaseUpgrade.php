...
class DatabaseUpgrade
{
    ...
    private function isIgnorableUpgradeException(PDOException $exception): bool
    {
        $errorInfo = $exception->errorInfo ?? [];
        $mysqlCode = null;
        if (isset($errorInfo[1])) {
            $mysqlCode = (int) $errorInfo[1];
        }

        if ($mysqlCode !== null) {
            $ignorableCodes = [1050, 1060, 1061, 1062, 1091, 1146, 1075];
            if (in_array($mysqlCode, $ignorableCodes, true)) {
                return true;
            }
        }
        ...
        foreach ($ignorableFragments as $fragment) {
    ...
    private function doUpgrade($number)
    {
        ...
        try {
            ...
            if ($this->isIgnorableUpgradeException($exception)) {
