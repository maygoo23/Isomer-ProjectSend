...
function try_queries($queries = [])
{
    ...
    foreach ($queries as $i => $value) {
        try {
            ...
            $params = !empty($queries[$i]["params"]) && is_array($queries[$i]["params"]) ? $queries[$i]["params"] : [];
            if (!empty($params)) {
...
function get_server_requirements_errors()
{
    ...
    if (empty($pdo_available_drivers)) {
        $errors_found[] = sprintf(
            __("Missing required extension: %s", "cftp_admin"),
            "pdo"
        );
    } else {
        ...
        if (
            DB_DRIVER == "mssql" &&
            !in_array("dblib", $pdo_available_drivers)
        ) {
            $errors_found[] = sprintf(
                __("Missing required extension: %s", "cftp_admin"),
                "pdo"
    ...
    $version_not_met = __(
        "%s minimum version not met. Please upgrade to at least version %s",
        "cftp_admin"
    );
...
function is_unique_username($string)
{
    ...
    $statement = $dbh->prepare(
        "SELECT * FROM " . TABLE_USERS . " WHERE user = :user"
    );
    $statement->execute([":user" => $string]);
...
{
    global $dbh;
    $allowed_custom_sort_columns = ["download_count"];
    ...
    if ($columns_query->rowCount() > 0) {
        ...
        $columns_keys = array_merge(
            $columns_keys,
            $allowed_custom_sort_columns
        );
        $orderby =
            isset($_GET["orderby"]) && in_array($_GET["orderby"], $columns_keys)
                ? $_GET["orderby"]
                : $column;
...
function get_downloads_information($id = null)
{
    ...
    $sql =
        "SELECT file_id, COUNT(*) as downloads, SUM( ISNULL(user_id) ) AS anonymous_users, COUNT(DISTINCT user_id) as unique_clients FROM " .
        TABLE_DOWNLOADS;
    if (!empty($id)) {
...
function download_information_exists($id)
{
    global $dbh;
    $statement = $dbh->prepare(
        "SELECT id FROM " . TABLE_DOWNLOADS . " WHERE file_id = :id"
    );
...
function get_client_by_username($client)
{
    ...
    $statement = $dbh->prepare(
        "SELECT id FROM " . TABLE_USERS . " WHERE user=:username"
    );
...
function get_user_by($user_type, $field, $value)
{
    ...
    $acceptable_fields = ["username", "name", "email"];

    if (in_array($field, $acceptable_fields)) {
        $statement = $dbh->prepare(
            "SELECT id FROM " . TABLE_USERS . " WHERE `$field`=:value"
        );
...
function get_file_by_filename($filename)
{
    global $dbh;
    $statement = $dbh->prepare(
        "SELECT * FROM " . TABLE_FILES . " WHERE url=:filename"
    );
    $statement->execute([
        ":filename" => $filename,
    ]);
...
function get_file_assignations($file_id)
{
    ...
    $statement = $dbh->prepare(
        "SELECT * FROM " . TABLE_FILES_RELATIONS . " WHERE file_id = :file_id"
    );
...
function render_footer_text()
{
    ...
    <footer>
        <div id="footer">
            <?php if (
                is_projectsend_installed() &&
                get_option("footer_custom_enable") == "1"
            ) {
                echo strip_tags(
                ...
                _e(
                    "Provided by",
                    "cftp_admin"
                ); ?> <a href="<?php echo SYSTEM_URI; ?>" target="_blank"><?php echo SYSTEM_NAME; ?></a> <?php if (
...
function message_no_clients()
{
    ...
    if (
        ...
        $msg =
            "<strong>" .
            __("Important:", "cftp_admin") .
            "</strong> " .
            __(
                "There are no clients or groups at the moment. You can still upload files and assign them later.",
                "cftp_admin"
            );
        $flash->warning($msg);
    }
...
function htmlentities_allowed(
    ...
    return strip_tags(
        $string,
        "<i><b><strong><em><p><br><ul><ol><li><u><sup><sub><s>"
    );
    /*
...
function htmlentities_allowed_code_editor(
    $html,
    $quoteStyle = ENT_COMPAT,
    $charset = CHARSET,
    $doubleEncode = false
...
function file_is_allowed($filename)
{
    ...
    $allowed_extensions = explode(
        ",",
        strtolower(get_option("allowed_file_types"))
    );
...
function make_thumbnail(
    ...
    if (!file_exists($file)) {
        $thumbnail_file =
            "thumb_unavailable_" . $width . "x" . $height . ".png";

        $thumbnail["original"]["url"] =
            ASSETS_IMG_URL . "/thumbnail-unavailable.png";
        $thumbnail["thumbnail"]["location"] =
            THUMBNAILS_FILES_DIR . DS . $thumbnail_file;
        $thumbnail["thumbnail"]["url"] =
            THUMBNAILS_FILES_URL . "/" . $thumbnail_file;
    ...
    if (!empty($thumbnail["thumbnail"]["location"])) {
        $image_info = @getimagesize($file);
        if (is_array($image_info) && !empty($image_info[0]) && !empty($image_info[1])) {
            $pixels = (int) $image_info[0] * (int) $image_info[1];
            $max_pixels = 20000000; // Skip thumbnail generation for extremely large images
            if ($pixels > $max_pixels) {
                $relative_path = str_replace(ROOT_DIR, '', $file);
                $relative_path = ltrim(str_replace('\\', '/', $relative_path), '/');
                $public_url = BASE_URI . $relative_path;
                $thumbnail["thumbnail"]["location"] = $file;
                $thumbnail["thumbnail"]["url"] = $public_url;
                $thumbnail["skipped_large_image"] = true;
                return $thumbnail;
            }
        }
    }
...
function generate_logo_url()
{
    ...
    if (file_exists($branding["dir"])) {
        ...
        if (file_is_image($branding["dir"])) {
            ...
            $branding["thumbnail"] = !empty($thumbnail["thumbnail"]["url"])
                ? $thumbnail["thumbnail"]["url"]
                : $branding["url"];
...
function get_branding_layout($return_thumbnail = false)
{
    ...
    $replace =
        ...
        $branding_image .
        '" alt="' .
        html_output(get_option("this_install_title")) .
        '" />';
...
function password_notes()
{
    ...
    if (count($rules_active) > 0) {
        $pass_notes_output =
            ...
            __("The password must contain, at least:", "cftp_admin") .
            "</strong><br />";
...
{
    $download_link =
        BASE_URI . "process.php?do=download&amp;id=" . $file_info["id"];
...
function option_file_upload(
    ...
    if (is_uploaded_file($file["tmp_name"])) {
         ...
         */
        if (
            !empty($validate_types) &&
            !preg_match($validate_types, strrchr($safe_filename, "."))
        ) {
            $continue = false;
        }
        ...
        if ($continue) {
            ...
            if (
                move_uploaded_file(
                    $file["tmp_name"],
                    ADMIN_UPLOADS_DIR . DS . $safe_filename
                )
            ) {
                if (!empty($option)) {
                ...
                $error = __(
                    "The file could not be moved to the corresponding folder.",
                    "cftp_admin"
                );
                $error .= __(
                    "This is most likely a permissions issue. If that's the case, it can be corrected via FTP by setting the chmod value of the",
                    "cftp_admin"
                );
                $error .= " " . ADMIN_UPLOADS_DIR . " ";
                $error .= __(
                    "directory to 755, or 777 as a last resource.",
                    "cftp_admin"
                );
                $error .= __(
                    "If this doesn't solve the issue, try giving the same values to the directories above that one until it works.",
                    "cftp_admin"
                );
            ...
            return [
                "status" => "error",
                "message" => __(
                    "The file you selected is not an allowed format.",
                    "cftp_admin"
                ),
            ];
        }
...
function file_editor_get_all_clients()
{
    ...
    $statement = $dbh->prepare(
        "SELECT id, name, level FROM " .
            TABLE_USERS .
            " WHERE level='0' AND account_requested='0' ORDER BY name ASC"
    );
    $statement->execute();
    $statement->setFetchMode(PDO::FETCH_ASSOC);
    while ($row = $statement->fetch()) {
...
function file_editor_get_clients_by_ids($clients_ids = [])
{
    ...
    $statement = $dbh->prepare(
        "SELECT id, name, level FROM " .
            TABLE_USERS .
            " WHERE level='0' AND account_requested='0' AND FIND_IN_SET(id, :clients_ids) ORDER BY name ASC"
    );
...
function file_editor_get_all_groups()
{
    ...
    $groups = [];
    $statement = $dbh->prepare(
        "SELECT id, name FROM " . TABLE_GROUPS . " ORDER BY name ASC"
    );
    $statement->execute();
    $statement->setFetchMode(PDO::FETCH_ASSOC);
    while ($row = $statement->fetch()) {
...
function file_editor_get_groups_by_members($clients_ids = [])
{
    ...
    $statement = $dbh->prepare(
        "SELECT DISTINCT group_id FROM " .
            TABLE_MEMBERS .
            " WHERE FIND_IN_SET(client_id, :clients_ids)"
    );
    $statement->bindParam(":clients_ids", $clients_ids);
    $statement->execute();
    $statement->setFetchMode(PDO::FETCH_ASSOC);
    ...
    $statement = $dbh->prepare(
        "SELECT id, name FROM " .
            TABLE_GROUPS .
            " WHERE FIND_IN_SET(id, :groups_ids) ORDER BY name ASC"
    );
...
function record_new_download($user_id = CURRENT_USER_ID, $file_id = null)
{
    ...
    if ($user_id == 0) {
        $statement = $dbh->prepare(
            "INSERT INTO " .
                TABLE_DOWNLOADS .
                " (file_id, remote_ip, remote_host, anonymous) VALUES (:file_id, :remote_ip, :remote_host, :anonymous)"
        );
        $statement->bindParam(":file_id", $file_id, PDO::PARAM_INT);
        ...
        $statement = $dbh->prepare(
            "INSERT INTO " .
                TABLE_DOWNLOADS .
                " (user_id , file_id, remote_ip, remote_host) VALUES (:user_id, :file_id, :remote_ip, :remote_host)"
        );
        ...
        if ($file->expires == "0" || $file->expired == false) {
            $statement = $dbh->prepare(
                "SELECT * FROM " .
                    ...
                    " WHERE (client_id = :client_id OR FIND_IN_SET(group_id, :groups)) AND file_id = :file_id AND hidden = '0'"
            );
...
function count_groups_requests_for_existing_clients()
{
    ...
    $statement = $dbh->prepare(
        "SELECT id FROM " .
            TABLE_USERS .
            " WHERE level='0' AND active='1' AND account_denied='0'"
    );
    $statement->execute();
    if ($statement->rowCount() > 0) {
        $statement->setFetchMode(PDO::FETCH_ASSOC);
    ...
    if (!empty($ignore_user_ids)) {
        ...
        $statement = $dbh->prepare(
            "SELECT id FROM " .
                TABLE_MEMBERS_REQUESTS .
                " WHERE denied='0' AND FIND_IN_SET(client_id, :ignore)"
        );
...
function recaptcha2_render_widget()
{
    if (recaptcha2_is_enabled()) { ?>
        <div class="form-group row">
            ...
            <div class="g-recaptcha" data-sitekey="<?php echo get_option(
                "recaptcha_site_key"
            ); ?>"></div>
        </div>
<?php }
...
function recaptcha2_get_request()
{
    ...
    if (recaptcha2_is_enabled()) {
        ...
        $recaptcha_request = file_get_contents(
            "https://www.google.com/recaptcha/api/siteverify?secret={$recaptcha_secret_key}&response={$recaptcha_response}&remoteip={$recaptcha_user_ip}"
        );
    }
...
function modify_url_with_parameters(
    $url,
    $parameters_add = [],
    $parameters_remove = []
) {
