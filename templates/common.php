...
if (
    file_exists(
        ROOT_DIR . DS . "templates" . DS . $this_template_slug . DS . "main.css"
    )
) {
    add_asset(
        "css",
        "ps_theme_css",
        BASE_URI . "templates/" . $this_template_slug . "/main.css",
        get_option("database_version"),
        "head"
    );
}
 ...
 */
$files_query =
    "SELECT id, file_id, client_id, group_id FROM " .
...
// Folders
$temp_files_query =
    "SELECT * FROM " . TABLE_FILES . " WHERE (FIND_IN_SET(id,:search_ids));";
...
$sql_sentence =
    "SELECT file_id, cat_id FROM " .
    TABLE_CATEGORIES_RELATIONS .
    " WHERE FIND_IN_SET(file_id, :files)";
...
if (!empty($found_all_files_array)) {
    ...
    if (isset($_GET["search"]) && !empty($_GET["search"])) {
        $files_query .=
            " AND (filename LIKE :title OR description LIKE :description)";
    ...
    while ($data = $sql_files->fetch()) {
        ...
        if ($add_file == true) {
            ...
            $my_files[$f] = [
                ...
                "id" => $data["id"],
                "url" => $data["url"],
                "save_as" => !empty($data["original_url"])
                    ? $data["original_url"]
                    : $data["url"],
