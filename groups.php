...
$page_title = __("Projects administration", "cftp_admin");
...
if (!empty($_GET["member"])) {
    ...
    if ($member != false) {
        $page_title =
            __("Projects where", "cftp_admin") .
            " " .
            html_entity_decode($member["name"]) .
            " " .
            __("is a contact", "cftp_admin");
        $member_exists = 1;
    ...
    if (!empty($_POST["batch"])) {
        ...
        switch ($_POST["action"]) {
            case "delete":
                ...
                if ($deleted_groups > 0) {
                    $flash->success(
                        __("The selected projects were deleted.", "cftp_admin")
                    );
                }
                break;
        }
    } else {
        $flash->error(__("Please select at least one project.", "cftp_admin"));
    }
    ...
    if (isset($no_results_error)) {
        switch ($no_results_error) {
            ...
            case "is_not_member":
                $flash->error(
                    __(
                        "There are no projects where this contact is listed.",
                        "cftp_admin"
                    )
                );
                break;
        ...
        $flash->warning(__("There are no projects created yet.", "cftp_admin"));
    }
}
        ...
        <div class="col-12">
            ...
            <?php if ($count > 0) {
                ...
                $thead_columns = [
                    ...
                    [
                        "content" => __("Contacts", "cftp_admin"),
                    ],
                    [
                ...
                while ($row = $sql->fetch()) {
                    ...
                    $tbody_cells = [
                        ...
                        [
                            "content" => !empty($group->members)
                                ? count($group->members)
                                : "0",
                        ],
                        [
                            "content" => !empty($group->files)
                                ? count($group->files)
                                : "0",
                        ],
                        [
