...
$filter_options_client_id = [
    "0" => __("client_id", "cftp_admin"),
];
global $dbh;
global $flash;
$sql_client_ids = $dbh->prepare(
    "SELECT client_id FROM " . TABLE_CUSTOM_DOWNLOADS . " GROUP BY client_id"
);
$sql_client_ids->execute();
$sql_client_ids->setFetchMode(PDO::FETCH_ASSOC);
while ($data_client_ids = $sql_client_ids->fetch()) {
    $filter_options_client_id[$data_client_ids["client_id"]] =
        $data_client_ids["client_id"];
}
    ...
    if (!empty($_POST["batch"])) {
        ...
        switch ($_POST["action"]) {
            case "activate":
                ...
                $flash->success(
                    __("The selected links were disabled.", "cftp_admin")
                );
                break;
            case "deactivate":
                foreach ($selected_files as $file_id) {
                ...
                $flash->success(
                    __("The selected links were enabled.", "cftp_admin")
                );
                break;
            case "delete":
                $delete_results = [
...
if ($query_table_files === true) {
    ...
    /**
     * If the user is a client, or a client is editing their files
    ...
    if (CURRENT_USER_LEVEL == "7" || CURRENT_USER_LEVEL == "0") {
        ...
        $params[":client_id"] = CURRENT_USER_ID; // FIXED: Use ID instead of USERNAME
    }
...
while ($data_uploaders = $sql_uploaders->fetch()) {
    if (is_null($data_uploaders["client_id"])) {
        continue;
    }
    $sql_cd = $dbh->prepare(
        "SELECT name FROM " . TABLE_USERS . " WHERE id=:client_id"
    );
    ...
    <form action="<?php echo $current_url; ?>" name="files_list" method="post" class="batch_actions">
            ...
            <div class="col-12">
                <?php if ($count_for_pagination > 0) {
                    ...
                    $conditions = [
                        "select_all" => true,
                        "is_not_client" =>
                            CURRENT_USER_LEVEL != "0" ? true : false,
                        "can_set_public" =>
                            CURRENT_USER_LEVEL != "0" ||
                    ...
                    while ($row = $sql->fetch()) {
                        ...
                        if (!$custom_download_uri) {
                            $custom_download_uri =
                                BASE_URI . "custom-download.php?link=";
                        } else {
                            // If custom URI is set but doesn't include base, add it
                            if (strpos($custom_download_uri, "http") !== 0) {
                                $custom_download_uri =
                                    BASE_URI . $custom_download_uri;
                            }
                        }
                        $custom_download_link =
                            $custom_download_uri . $custom_download->link;
                        ...
                        if ($custom_download->client_id) {
                            ...
                            $usersql->execute([
                                "client_id" => $custom_download->client_id,
                            ]);
                        ...
                        $link_copy_button =
                            '<a href="#" class="btn btn-pslight btn-sm" onclick="navigator.clipboard.writeText(\'' .
                            $custom_download_link .
                            '\'); alert(\'Link copied!\');">
        <i class="fa fa-copy" style="cursor: pointer"></i>
                        ...
                        $tbody_cells = [
                            ...
                            [
                                "content" =>
                                    $file_edit_button . $link_copy_button,
                            ],
                        ];
